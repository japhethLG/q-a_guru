    ...
    return (
        <div className="p-4 bg-gray-800 rounded-lg shadow-lg flex flex-col h-full">
            <div className="flex justify-between items-center mb-3">
                ...
                <button onClick={onSave} className="p-2 hover:bg-gray-700 rounded-md" title="Save current version">
            ...
            <div className="flex-grow overflow-y-auto pr-2 -mr-2">
                {versions.length > 0 ? (
                            ...
                            return (
                                <li
                                    ...
                                    className={`p-3 rounded-md transition-all duration-200 cursor-pointer border-l-4 ${
                                        isPreviewing ? 'bg-gray-600 border-cyan-400 ring-2 ring-cyan-400' 
                                        : isCurrent ? 'bg-gray-700/80 border-cyan-600'
                                        : 'bg-gray-700 border-transparent hover:bg-gray-600/80 hover:border-gray-500'
                                    }`}
                                >
                                    <div className="flex justify-between items-center gap-2">
                                        <div className="overflow-hidden">
                                            <p className="text-sm font-medium text-gray-200">{new Date(version.timestamp).toLocaleString()}</p>
                                            <p className="text-xs text-gray-400 mt-1 italic truncate" title={version.reason}>
                                                {version.reason}
                                            </p>
                                        </div>
                                        <div className="flex items-center space-x-2 flex-shrink-0">
                                            <button 
                                                onClick={(e) => { e.stopPropagation(); handleRevertClick(version.id); }}
                                            ...
                                            <button
                                                 onClick={(e) => { e.stopPropagation(); handleDeleteClick(version.id); }}
                    ...
                    <p className="text-center text-gray-500 text-sm py-8">Generate a document to start version history.</p>
                )}
            </div>
        </div>
    );
