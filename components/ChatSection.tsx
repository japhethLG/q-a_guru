...
export const ChatSection: React.FC<ChatSectionProps> = ({ documentsContent, documentHtml, selectedText, onDocumentEdit }) => {
        ...
        try {
            ...
            if (functionCalls && functionCalls.length > 0) {
                ...
                if (editCall && editCall.args) {
                    const { full_document_html, html_snippet_to_replace, replacement_html } = editCall.args;
                    ...
                    if (full_document_html) {
                        ...
                        if (documentHtml.includes(html_snippet_to_replace as string)) {
                           newHtml = documentHtml.replace(html_snippet_to_replace as string, replacement_html as string);
                        } else {
                            console.warn("AI tried to replace a snippet that wasn't found.");
                            setMessages(prev => [...prev, {role: 'model', content: "I tried to make an edit, but couldn't find the exact text to change. Try highlighting it first."}]);
                            setIsLoading(false);
                            return; 
                        }
                    }
                    ...
                    if (newHtml) {
                       ...
                       setMessages(prev => [...prev, {role: 'system', content: `Document updated: "${messageToSend}"`}]);
                    }
                }
            } else {
    ...
    const actionButtons = [
        ...
        { label: 'Fix Grammar', prompt: 'Fix spelling & grammar for the selected text.' },
    ...
    return (
        <div className="flex flex-col h-full bg-gray-800 rounded-lg shadow-lg overflow-hidden">
            ...
            <div className="flex-grow p-3 space-y-3 overflow-y-auto">
                {messages.length === 0 && !isLoading && (
                    <div className="flex flex-col items-center justify-center h-full text-center text-gray-500">
                         ...
                         <p className="text-sm">Select text in the editor to get contextual actions, or type a general question below.</p>
                    </div>
                )}
                {messages.map((msg, index) => (
                    <div key={index} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                       {msg.role === 'system' ? (
                         ...
                         <div className={`max-w-xs md:max-w-md lg:max-w-xs xl:max-w-sm px-3 py-2 rounded-lg ${msg.role === 'user' ? 'bg-cyan-600 text-white' : 'bg-gray-700 text-gray-200'}`}>
            ...
            <div className="p-3 border-t border-gray-700 bg-gray-800/80 backdrop-blur-sm">
                 <ContextDisplay contextText={contextText} onClear={() => setContextText('')} />
                {contextText && (
                    <div className="grid grid-cols-3 gap-2 mb-2">
                        {actionButtons.map(btn => (
                            <button key={btn.label} onClick={() => handleSendMessage(btn.prompt)} disabled={isLoading} className="text-xs bg-gray-700 hover:bg-gray-600 disabled:bg-gray-600/50 p-2 rounded-md transition-colors">
                                {btn.label}
                            </button>
                        ))}
                    </div>
                ...
                <div className="flex gap-2">
                    <input
                        ...
                        disabled={isLoading}
                    />
                    <button onClick={() => handleSendMessage()} disabled={isLoading || !input.trim()} className="bg-cyan-600 hover:bg-cyan-700 disabled:bg-gray-500 disabled:cursor-not-allowed text-white font-bold p-2 rounded-md transition-colors flex items-center justify-center w-20">
