...
export const FileUploadSection: React.FC<FileUploadSectionProps> = ({ files, onFileChange, setFiles, setDocumentsContent, isLoading }) => {
    ...
    return (
        <div className="p-4 bg-gray-800 rounded-lg shadow-lg">
            <h3 className="text-lg font-semibold mb-3 text-cyan-400">1. Upload Documents</h3>
            <div className="relative border-2 border-dashed border-gray-600 rounded-lg p-6 text-center hover:border-cyan-500 transition-colors">
                ...
                <label htmlFor="file-upload" className="mt-2 text-sm font-medium text-gray-300 cursor-pointer">
                ...
                <input id="file-upload" type="file" multiple className="sr-only" onChange={onFileChange} accept=".pdf,.docx,.pptx,.txt" />
            </div>
            <div className="mt-4 space-y-2">
                {files.map((file, index) => (
                    <div key={index} className="flex items-center justify-between bg-gray-700 p-2 rounded-md">
                        ...
                        <button onClick={() => removeFile(index)} className="p-1 hover:bg-red-500/20 rounded-full">
                            <XIcon className="h-4 w-4 text-red-400" />
                        </button>
                    </div>
                ))}
                {isLoading && files.length > 0 && <p className="text-sm text-cyan-400 flex items-center gap-2"><LoaderIcon className="h-4 w-4" /> Parsing files...</p>}
            </div>
        </div>
    );
};
